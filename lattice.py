...
class lattice():
    ...
    def __init__(self, num_rayos, Lx, Ly, nx, ny, n0, n1, loc_eje=None, A=None, caos=None, random_theta=False, chosen_theta=None):

        self.error = False #bool para si pasa algun error
        ...
        self.inicializar(loc_eje=loc_eje, A=A, caos=caos, random_theta=random_theta, chosen_theta=chosen_theta)
    ...
    def inicializar(self, loc_eje=None, A=None, caos=None, random_theta=False, chosen_theta=None):
        ...
        self.idxx_rayos = []; self.idxy_rayos = []
        self.coordx_rayos = []; self.coordy_rayos = []
    ...
    def actualizacion(self, i, idxx, idxy, cx, cy, dx, dy, ix, iy, theta1, mov_type):
      ...
      #  print('donde', idxx, idxy)
      #  print('hacia', ix, iy, mov_type)
      #  print('caracteristicas', theta1_orig, cx, cy, 'ds', dx, dy)
        ...
        if 0 <= ix <= self.nx-1 and 0 <= iy <= self.ny-1:
            ...
            if theta1 < thetac or n2>n1: #no reflexión total
              ...
              #  print('Ángulo 2 normal', theta2*180/np.pi)
    ...
    def evolucion(self):
        ...
        for i in tqdm(range(self.num_rayos)):
             ...
             while self.movimiento[i] != 0: #si no ha llegado a los extremos del cubo
                ...
                cx, cy = self.coordx_rayos[i][-1], self.coordy_rayos[i][-1] #que coordenadas dentro de la celda
                th = self.theta[i][-1] #que ángulo respecto la horizontal

                # CALCULAMOS CONTACTOS
                dx, dy, ix, iy, mov_type = calcular_contactos(self.dx, self.dy, cx, cy, idxx, idxy, th)
